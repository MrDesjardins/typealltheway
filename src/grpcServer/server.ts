...
import { IProfileServiceServer, ProfileServiceService } from '../../autogenerated/src/grpcServer/protobuf/profile_grpc_pb';
import { ProfileRequest, ProfileResponse, Profile, ProfilesRequest, ProfilesResponse, ListProfilesRequest } from '../../autogenerated/src/grpcServer/protobuf/profile_pb';
...
class ProfileServer implements IProfileServiceServer {
  ...
  public getProfiles(call: grpc.ServerUnaryCall<ProfilesRequest>, callback: grpc.sendUnaryData<ProfilesResponse>): void {
    const request = call.request;
    const ids = request.getIdsList();
    const result = Object.values(profiles).filter((d: Profile.AsObject) => ids.indexOf(d.id) >= 0).map(f => {
  ...
  public listProfiles(call: grpc.ServerUnaryCall<ListProfilesRequest>, callback: grpc.sendUnaryData<ProfilesRequest>): void {
